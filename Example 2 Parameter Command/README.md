...
Whatever someone types after a "normal" chat command is put into a `%rawInput%` variable for your Action to us.
...
The key parts here the "Mode" which is set to "Regex" and then the value of the "Regex" field.
In my example, the field contains:
...
the first paramater will be available via a `%param1%` variable and the second parameter will be available via `%param2%`.
...
Within this group, there is another group `(?<param2>\S+))`. This should look familiar to the first parameter that was captured.
